      ...
      <nav className="navbar justify-between bg-primary">
        ...
        {user ? (
          <>
            <div>
              <GoldGlass />
              <h3 className="text-3xl font-bold px-2">{user.points}</h3>
              <div className="w-8 h-8 rounded-full shrink-0 ring ring-accent mx-3">
                <img src={user.avatar} alt="user avatar" className="rounded-full" />
              </div>
              <h3 className="self-center text-xl m-0">{user.displayName}</h3>
              <SignOutButton />
            </div>
          </>
        ) : null}
